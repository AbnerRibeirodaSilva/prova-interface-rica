...
class FormularioContaViewModel(
    ...
    private fun formularioValido(): Boolean {
        state = state.copy(
            ...
            valor = state.valor.copy(
                codigoMensagemErro = validarValorNumerico(state.valor.valor)
            )
        )
    ...
    private fun validarValorNumerico(valor: String): Int {
        if (valor.isBlank()) {
            return R.string.required_value
        }

        return try {
            val valorNumerico = BigDecimal(valor)

            when {
                valorNumerico <= BigDecimal.ZERO -> R.string.positive_value
                else -> 0
            }
